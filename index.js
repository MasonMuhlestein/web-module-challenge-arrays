...
function copy(originalFlavors){
    ...
    {
        Flavorscopy.push(originalFlavors[i])
    }
    return Flavorscopy
}    
...
function removeFlavorByName(originalFlavors, f){
    for (let i = 0; i < originalFlavors.length; i++)
    {
        if (originalFlavors[i] === f){
            originalFlavors.splice(i, i++);
        }        
    }
    return originalFlavors;
}
...
const newList = [];
    ...
    for(let i = 0; i < originalFlavors.length; i++)
    {
        ...
        {
            newList.push(originalFlavors[i])
        }
    }
    return newList;
}
